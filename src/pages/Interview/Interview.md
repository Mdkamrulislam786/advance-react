...
Answer:
By using PureComponent, shouldComponetUpdate lifecycle methode, useMemo, useCallback, React.Memo(Compoent) etc. This check if the prev state is similar to the next state if it is, then component dosent re-render and memo calculates the state so it doesnt re-render every time.
...
#### 28) Why should you dispatch an action to update the state and not update store dierctly?
